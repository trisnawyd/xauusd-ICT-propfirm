...
- Daily loss limit: if `get_daily_drawdown` total_pnl ≤ −$200 (Top One Trader 2 Step PRO v2 $5K = 4% of initial balance, static — see `prop-rules/toponetrader-2step-pro-5k.md`) → HARD BLOCK all trades, output warning
...
- **BROKER TIME vs UTC RULE — MCP timestamps are NOT UTC.** `get_ohlcv`, `detect_structure`, `detect_order_blocks`, `detect_fvg`, `detect_liquidity_sweeps` **and `get_economic_calendar`** all return timestamps in **BROKER time (Exness = UTC+3)**. `get_session_levels` is the ONLY tool that returns true UTC (`UTC:12:33`).
  - ⚠️ **`get_economic_calendar` is the most dangerous one** — it prints event times in broker time with no label (`15:30 [HIGH] CPI`). **A news time read straight off this tool will be 3h wrong.** US CPI/NFP/PPI at `15:30` = **12:30 UTC**; FOMC at `21:00` = **18:00 UTC**. Convert EVERY event before writing a news window or an avoid-window.
  - **Convert before writing any time into an analysis, alert, session label, or news window: UTC = broker time − 3h.** (Broker `15:30` = UTC `12:30`.)
  - **Sanity check that catches it every time:** compare any candle's stamp against `get_session_levels` UTC in the same tool turn. If the candle reads 3 hours ahead, it is broker time — convert it.
  - **Never label a broker timestamp as UTC.** (2026-07-14: the CPI news file wrote "fires 15:30 UTC" — broker time copied verbatim. Real release was 12:30 UTC. The avoid-window and the pre-release operator deadline both pointed 3 hours PAST the event, which had already spiked gold +73 points through a resting short.)
  - Session identification (Asian/London/NY killzones) is UTC-defined — a broker-time stamp will mislabel the session by one whole block.
- **NEWS DIRECTION RULE — never let a standing HTF bias author the pre-release call.** On HIGH-impact two-way events, map both branches with symmetric detail and take NO lean unless there is evidence independent of the current bias/open exposure. Phrases like "the pain trade is X, which aligns with our bias" or "news risk is skewed in our favour" are thesis-defence, not analysis — delete and rewrite. (2026-07-14 CPI: a bear map + a resting short produced a confident "hawkish surprise" call; every number printed cold and gold went vertical.) If a pending/position is exposed to the release, state the exposure and the decision deadline **in UTC** — and treat "carry it" as requiring the same justification as opening it fresh.
...
3. Call `get_daily_drawdown` — verify daily loss limit not breached (−$200 / 4% of $5K initial)
