...
export function validateExecution(params) {
    ...
    if (!EXECUTION_TOOLS.has(name))
        return null; // not an execution tool
...
export function analyzeStructure(swings) {
    const events = [];
    let trend = "UNKNOWN";
    let prevHigh = null, lastHigh = null;
    let prevLow = null, lastLow = null;
...
export function detectFVGs(candles, minSize = 0) {
    const fvgs = [];
...
export function findOrderBlocks(candles, events) {
    const obs = [];
...
function compactAnalysis(toolName, data) {
    switch (toolName) {
        case "detect_structure": {
            const lines = [`${data.timeframe} trend:${data.trend}`];
            for (const e of data.events)
                lines.push(`${e.type} ${e.direction}@${e.price} broke ${e.broken_level} ${fmtT(e.time)}`);
        ...
        case "detect_fvg": {
            ...
            for (const f of data.fvgs)
                lines.push(`${f.type} ${f.bottom}-${f.top} ${fmtT(f.time)} [${f.mitigated ? "filled" : "open"}]`);
            return lines.join("\n");
        }
        case "detect_order_blocks": {
            ...
            for (const o of data.order_blocks)
                lines.push(`${o.type} ${o.bottom}-${o.top} ${fmtT(o.time)} [${o.mitigated ? "filled" : "open"}] bos:${o.bos_price}@${fmtT(o.bos_time)}`);
            return lines.join("\n");
        }
        ...
        case "detect_liquidity_sweeps": {
            ...
            for (const s of data.sweeps)
                lines.push(`${s.type}@${s.swept_level} wick:${s.wick} close:${s.close} ${fmtT(s.time)}`);
            return lines.join("\n");
        }
        default:
...
async function handleAnalysisTool(name, args) {
    const tf = args.timeframe || "M15";
    const count = args.count || 100;
    switch (name) {
        case "detect_structure": {
            const swingData = await callEA("get_swing_levels", {
                timeframe: tf,
                count: args.swing_count || 20,
            });
        ...
        case "get_premium_discount": {
            ...
            const highs = swingData.swings.filter((s) => s.type === "high");
            const lows = swingData.swings.filter((s) => s.type === "low");
            const swingHigh = highs.length
                ? Math.max(...highs.map((h) => h.price))
                : 0;
            const swingLow = lows.length ? Math.min(...lows.map((l) => l.price)) : 0;
            const result = calculatePremiumDiscount(swingHigh, swingLow, tickData.bid);
            return compactAnalysis(name, { timeframe: tf, ...result });
        }
        case "detect_liquidity_sweeps": {
            const swingData = await callEA("get_swing_levels", {
        ...
        case "get_performance_stats": {
            ...
            let wins = 0, losses = 0, grossWin = 0, grossLoss = 0, totalDuration = 0;
            const sessions = {
                ASIAN: { w: 0, l: 0, pnl: 0 },
            ...
            for (const t of trades) {
                ...
                const hour = parseInt(t.open_time?.split(" ")[1]?.split(":")[0] ?? "0");
                let ses = "ASIAN";
                if (hour >= 13 && hour < 17)
                    ses = "OVERLAP";
            ...
            for (let i = trades.length - 1; i >= 0; i--) {
                const n = trades[i].net_profit;
                if (i === trades.length - 1) {
                    streakType = n > 0 ? "W" : "L";
                    streak = 1;
                }
                else if ((n > 0 && streakType === "W") ||
                    (n <= 0 && streakType === "L"))
                    streak++;
                else
                    break;
            }
            const sesFmt = (s) => `${s.w}W/${s.l}L $${s.pnl.toFixed(2)}`;
            return [
...
const server = new Server({ name: "mt5-bridge", version: "1.0.0" }, { capabilities: { tools: {}, resources: {} } });
export const TOOL_DEFINITIONS = [
        ...
        inputSchema: {
            ...
            properties: {
                ...
                timeframe: {
                    ...
                    description: "Candle timeframe (e.g. M1, M5, M15, H1, H4, D1, W1)",
                },
                count: {
                    type: "number",
    ...
    },
    {
        ...
        description: "Get all pending orders (BUY_LIMIT, SELL_LIMIT, BUY_STOP, SELL_STOP) on XAUUSD",
        inputSchema: { type: "object", properties: {} },
    },
    {
    ...
    },
    {
        ...
        description: "Get closed trade history. period: 'today', '7d' (default), '30d', 'all'. Returns ticket, type, open/close price, profit, commission, swap, net_profit, duration_minutes.",
        inputSchema: {
            type: "object",
            properties: {
                period: {
    ...
    },
    {
        ...
        description: "Get Asia/London/NY session levels (high, low, open) in UTC. Returns active_session and levels for each session.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "get_swing_levels",
        description: "Get last N swing highs/lows on a given timeframe. Returns alternating swings with price and time.",
        inputSchema: {
            type: "object",
            properties: {
                timeframe: {
    ...
    },
    {
        ...
        description: "Get recent tick spread data for XAUUSD with avg/max/min stats. Spread is in pips.",
        inputSchema: {
            type: "object",
            properties: {
                count: {
    ...
    },
    {
        ...
        description: "Calculate recommended lot size from SL pips and risk %. Returns recommended_lot, risk_dollars, pip_value. For XAUUSD 0.01 lot.",
        inputSchema: {
            type: "object",
            properties: {
                sl_pips: {
    ...
    },
    {
        ...
        description: "Get today's closed P&L + floating P&L from open positions since UTC midnight. Returns closed_pnl, floating_pnl, total_pnl.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "place_order",
        description: "Place a market order on XAUUSD. REQUIRES user confirmation before calling. " +
    ...
    {
        name: "close_position",
        description: "Close an open position by ticket, or close all XAUUSD positions. " +
    ...
    },
    {
        ...
        description: "Place a pending order on XAUUSD (limit or stop). REQUIRES user confirmation. " +
        ...
        inputSchema: {
            ...
            properties: {
                type: {
                    ...
                    description: "BUY_LIMIT=buy at lower price, SELL_LIMIT=sell at higher price, BUY_STOP=buy on breakout up, SELL_STOP=sell on breakout down",
                },
                price: {
                    type: "number",
    ...
    {
        name: "trailing_stop",
        description: "Trail the stop loss of an open position by N pips from current price. Only moves SL in the profitable direction.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Move stop loss to break-even (open price + optional buffer pips). No-op if SL already at or past BE.",
        inputSchema: {
            type: "object",
            properties: {
                ticket: { type: "number", description: "Position ticket" },
    ...
    // --- Phase 4B: Performance stats (computed locally from trade history) ---
    {
        ...
        description: "Compute trading statistics from closed trade history: win rate, profit factor, avg win/loss, session breakdown, current streak.",
        inputSchema: {
            type: "object",
            properties: {
                period: {
    ...
    // --- Phase 5D: Economic calendar ---
    {
        ...
        description: "Get upcoming USD high/medium-impact economic events from MT5 calendar. Returns event name, time (UTC), impact level, forecast, and prev value.",
        inputSchema: {
            type: "object",
            properties: {
                period: {
                    type: "string",
                    enum: ["today", "24h", "48h"],
                    description: "Time window: today (UTC day), 24h from now (default), 48h from now",
                },
                high_only: {
                    type: "string",
                    enum: ["true", "false"],
                    description: "Set true to return only HIGH-impact events (default: false = HIGH + MEDIUM)",
                },
            },
            required: [],
        },
    ...
    {
        name: "set_alert",
        description: "Set a price alert on XAUUSD. Auto-detects direction (above/below) from current price if not specified. EA fires Print alert when triggered, then removes it.",
        inputSchema: {
            type: "object",
            properties: {
                ...
                label: {
                    type: "string",
                    description: "Short label for this alert (e.g. 'OB top', 'Asian high')",
                },
                direction: {
                    type: "string",
                    enum: ["above", "below"],
                    description: "Fire when price goes above or below (auto-detected if omitted)",
                },
            },
            required: ["price"],
        },
    ...
    {
        name: "list_alerts",
        description: "List all active price alerts currently registered in the EA.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "delete_alert",
    ...
    // --- Phase 2: Analysis tools (computed locally in server.ts) ---
    {
        ...
        description: "Detect BoS (Break of Structure) and CHoCH (Change of Character) events. Returns trend direction and list of structure events with price/time.",
        inputSchema: {
            type: "object",
            properties: {
                timeframe: {
    ...
    {
        name: "detect_fvg",
        description: "Detect Fair Value Gaps (imbalance zones) from 3-candle patterns. Returns bullish/bearish FVGs with mitigated/unmitigated status.",
        inputSchema: {
            type: "object",
            properties: {
                timeframe: {
    ...
    },
    {
        ...
        description: "Detect Order Blocks — last opposing candle before each BoS/CHoCH event. Returns OB zones with mitigation status.",
        inputSchema: {
            type: "object",
            properties: {
                timeframe: {
    ...
    },
    {
        ...
        description: "Calculate premium/discount zones, equilibrium, and OTE (61.8%-78.6% retracement) from the swing range. Returns current zone classification.",
        inputSchema: {
            type: "object",
            properties: {
                timeframe: {
    ...
    },
    {
        ...
        description: "Detect BSL (buy-side) and SSL (sell-side) liquidity sweeps — wicks exceeding swing levels that close back inside.",
        inputSchema: {
            type: "object",
            properties: {
                timeframe: {
...
export function compactOutput(toolName, raw) {
    try {
        const d = JSON.parse(raw);
        const fmtTime = (timeStr, singleDay) => {
            const [datePart, timePart] = timeStr.split(" ");
            const hhmm = timePart?.substring(0, 5) ?? "00:00";
            if (singleDay)
                return hhmm;
        ...
        switch (toolName) {
            case "get_session_levels": {
                const t = d.time_utc.split(" ")[1]?.substring(0, 5) ?? "";
                const a = d.asian, l = d.london, ny = d.new_york;
                return [
            ...
            case "get_ohlcv": {
                ...
                const rows = candles.map((c) => `${fmtTime(c.time, singleDay)},${c.open},${c.high},${c.low},${c.close}`);
            ...
            case "get_swing_levels": {
                ...
                const singleDay = dates.size === 1;
                const rows = swings.map((s) => `${fmtTime(s.time, singleDay)} ${s.type === "high" ? "H" : "L"}${s.price}`);
            ...
            case "get_open_positions": {
                ...
                return positions
                    .map((p) => `#${p.ticket} ${p.type} ${p.symbol} @${p.open_price} lot:${p.volume} sl:${p.sl} tp:${p.tp} pnl:${p.profit}`)
                    .join("\n");
            }
            case "get_pending_orders": {
                ...
                return orders
                    .map((o) => `#${o.ticket} ${o.type} @${o.price} lot:${o.volume} sl:${o.sl} tp:${o.tp}`)
                    .join("\n");
            }
            ...
            case "get_trade_history": {
                ...
                if (trades.length === 0)
                    return "no trades";
                const rows = trades.map((t) => `${t.ticket},${t.type},${t.open_price},${t.close_price},${t.profit},${t.net_profit},${t.duration_minutes}`);
            ...
            case "list_alerts": {
                ...
                const rows = d.alerts.map((a) => `#${a.id} ${Number(a.price).toFixed(2)} [${a.direction}] "${a.label}"`);
            ...
            case "get_economic_calendar": {
                ...
                const rows = d.events.map((e) => `${e.time.split(" ")[1]?.substring(0, 5)} [${e.impact}] ${e.event} | fcst:${e.forecast} prev:${e.prev}`);
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    if (ANALYSIS_TOOLS.has(req.params.name)) {
        const text = await handleAnalysisTool(req.params.name, (req.params.arguments ?? {}));
        return { content: [{ type: "text", text }] };
    }
    // 3B: Daily loss limit hard block — checked before any execution tool (except cancel)
    if (EXECUTION_TOOLS.has(req.params.name) &&
