...
describe("compactOutput: alert tools", () => {
    describe("set_alert", () => {
        it("formats a valid response with 2-decimal price", () => {
            ...
            expect(compactOutput("set_alert", raw)).toBe('ALERT #1 set: "OB top" @ 2500.50 (above) | active:1');
        });
        it("preserves trailing zeros even when JSON.parse strips them", () => {
            ...
            expect(compactOutput("set_alert", raw)).toBe('ALERT #2 set: "round" @ 2500.00 (above) | active:2');
        });
        it("returns ERROR on error field", () => {
            expect(compactOutput("set_alert", json({ error: "price required" }))).toBe("ERROR: price required");
        });
    });
    describe("list_alerts", () => {
        it("returns empty-state message when count is 0", () => {
            expect(compactOutput("list_alerts", json({ count: 0, alerts: [] }))).toBe("no active alerts");
        });
        ...
        it("guards against missing alerts array", () => {
            expect(compactOutput("list_alerts", json({ count: 1 }))).toBe("no active alerts");
        });
    });
    describe("delete_alert", () => {
        it("formats successful deletion with 2-decimal price", () => {
            ...
            expect(compactOutput("delete_alert", raw)).toBe('ALERT #3 deleted: "x" @ 2500.50');
        });
        it("returns ERROR on not-found", () => {
            expect(compactOutput("delete_alert", json({ error: "alert id 99 not found" }))).toBe("ERROR: alert id 99 not found");
        });
    });
});
describe("compactOutput: data query tools", () => {
    it("get_current_tick: single-line format", () => {
        ...
        expect(compactOutput("get_current_tick", raw)).toBe("XAUUSD bid:2500.5 ask:2500.7 spd:2");
    });
    it("get_account_info: single-line format", () => {
        ...
        expect(compactOutput("get_account_info", raw)).toBe("bal:10000 eq:10050 margin:100 free:9950");
    });
    it("get_spread_history: stats line", () => {
        const raw = json({ avg: 1.2, max: 2.5, min: 0.5, count: 20 });
        expect(compactOutput("get_spread_history", raw)).toBe("spread avg:1.2 max:2.5 min:0.5 n:20");
    });
    it("calculate_lot_size: single-line format", () => {
        ...
        expect(compactOutput("calculate_lot_size", raw)).toBe("lot:0.01 risk:$10 pip_val:0.1");
    });
    it("get_daily_drawdown: single-line format", () => {
        const raw = json({ closed_pnl: -5, floating_pnl: 2, total_pnl: -3 });
        expect(compactOutput("get_daily_drawdown", raw)).toBe("closed:-5 float:2 total:-3");
    });
    ...
    describe("get_swing_levels", () => {
        it("formats H/L prefixed swings (single-day)", () => {
            ...
            expect(compactOutput("get_swing_levels", raw)).toBe(["M15 swings", "09:00 H2510", "10:30 L2495"].join("\n"));
        });
    });
    describe("get_open_positions", () => {
        it("no positions", () => {
            expect(compactOutput("get_open_positions", json([]))).toBe("no open positions");
            expect(compactOutput("get_open_positions", json({ positions: [] }))).toBe("no open positions");
        });
        it("formats each position (bare array)", () => {
            ...
            expect(compactOutput("get_open_positions", raw)).toBe("#111 BUY XAUUSD @2500 lot:0.01 sl:2490 tp:2520 pnl:5");
        });
        it("formats each position (wrapped shape)", () => {
            ...
            expect(compactOutput("get_open_positions", raw)).toBe("#222 SELL XAUUSD @2510 lot:0.01 sl:2520 tp:2490 pnl:-3");
        });
    });
    describe("get_pending_orders", () => {
        it("no orders (bare array and wrapped)", () => {
            expect(compactOutput("get_pending_orders", json([]))).toBe("no pending orders");
            expect(compactOutput("get_pending_orders", json({ orders: [] }))).toBe("no pending orders");
        });
        it("formats pending orders", () => {
            ...
            expect(compactOutput("get_pending_orders", raw)).toBe("#333 BUY_LIMIT @2495 lot:0.01 sl:2485 tp:2515");
        });
    });
    describe("get_trade_history", () => {
        it("no trades", () => {
            expect(compactOutput("get_trade_history", json([]))).toBe("no trades");
            expect(compactOutput("get_trade_history", json({ trades: [] }))).toBe("no trades");
        });
    ...
    describe("get_economic_calendar", () => {
        it("error case", () => {
            expect(compactOutput("get_economic_calendar", json({ error: "calendar unavailable" }))).toBe("ERROR: calendar unavailable");
        });
        it("no events", () => {
            expect(compactOutput("get_economic_calendar", json({ count: 0, period: "24h" }))).toBe("no USD high/medium events in next 24h");
        });
        it("formats event rows", () => {
            ...
            expect(compactOutput("get_economic_calendar", raw)).toBe(["USD events (24h): 1", "13:30 [HIGH] CPI m/m | fcst:0.3% prev:0.2%"].join("\n"));
        });
    });
});
...
//# sourceMappingURL=compact-output.test.js.map