...
import type { Direction, ScalpEntry } from "@/lib/types";
...
export default function ScalpListPage() {
  const items = getScalpList();
  const latest = items[0];
  const latestDoc = latest ? getScalp(latest.date, latest.slug) : null;

  const byDate = new Map<string, ScalpEntry[]>();
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-2xl font-bold tracking-tight">Scalp Analysis</h1>
      ...
      {latest && latestDoc && (
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Link
              href={`/scalp/${latest.date}/${latest.slug}`}
            ...
            <CardContent className="space-y-5 py-5">
              ...
              {(latestDoc.entry.direction === "LONG" || latestDoc.entry.direction === "SHORT") && (
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
                  ...
                  <Stat label="TP1" value={latestDoc.entry.tp} />
                  <Stat label="TP2" value={latestDoc.entry.tp2} />
                  <Stat
                    label="R:R"
